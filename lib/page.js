        ...
        if (message.method.match(/^Network\./)) {
            ...
            case 'Network.dataReceived':
                if (id in self.objects) {
                    ...
                    if (!self.objects[id].cached) {
                        self.objects[id].dataArrivals.push(
                        {
                            'timestamp': (message.params.timestamp - self.objects[id].responseMessage.timestamp)*1000 + (self.objects[id].responseMessage.response.timing.receiveHeadersEnd - self.objects[id].responseMessage.response.timing.requestTime*1000),
