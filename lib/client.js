...
function checkEnd(c, url, obj, page) {
	c.Runtime.evaluate({'expression': "performance.getEntriesByName('"+url+"')[0].responseEnd;", 'returnByValue': true}, function (error, response){
		common.dump('--- RT: responseEnd: '+ response.result.value + " resFinMs: " + (obj.responseFinishedMs - page.originalRequestMs)  + ' for ' + url);
    });
}
